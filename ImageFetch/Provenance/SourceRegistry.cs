...
/// whether that template already carries its own licence card. <c>{{Geograph}}</c>,
/// <c>{{Geograph Deutschland}}</c> and <c>{{Wikimapia}}</c> bake in CC-BY-SA, so a fetched image
/// from them needs no separate licence card; <c>{{Flickr}}</c> and <c>{{Wikimedia Commons}}</c> do.
/// </summary>
...
public static class SourceRegistry
{
    ...
    public const string GeographDe = "geograph-de";
    ...
    private static readonly Dictionary<string, SourceInfo> ByKey = new()
    {
        ...
        [GeographDe] = new SourceInfo("Geograph Deutschland", "Geograph Deutschland", SelfLicensing: true),
